...
import { StaffMember, AttendanceStatus } from "@/data/staff";
import { Check, X, Hash, Briefcase, Building2 } from "lucide-react";

interface StaffSwipeCardProps {
  member: StaffMember;
...
export default function StaffSwipeCard({ member, onSwipe }: StaffSwipeCardProps) {
    ...
    >
      <div className="relative overflow-hidden rounded-2xl bg-card shadow-xl border border-border">
        ...
        <div className="px-6 pt-10 pb-8 flex flex-col items-center text-center space-y-5">
          ...
          {/* Name */}
          <h2 className="text-2xl font-bold text-card-foreground tracking-tight">{member.name}</h2>
          ...
          <div className="w-full grid grid-cols-1 gap-3">
            <div className="flex items-center gap-3 rounded-xl bg-muted/50 px-4 py-3">
              ...
              <div className="text-left">
                <p className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground/70">Staff ID</p>
                <p className="text-sm font-semibold text-card-foreground">{member.staffId}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-xl bg-muted/50 px-4 py-3">
              <Briefcase className="h-4 w-4 text-muted-foreground shrink-0" />
              <div className="text-left">
                <p className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground/70">Role</p>
            ...
            <div className="flex items-center gap-3 rounded-xl bg-muted/50 px-4 py-3">
              ...
              <div className="text-left">
                <p className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground/70">Department</p>
                <p className="text-sm font-semibold text-card-foreground">{member.department}</p>
              </div>
            </div>
          </div>
        </div>
...
export function StaffSwipeActions({
  onPresent,
  onAbsent,
}: {
  onPresent: () => void;
